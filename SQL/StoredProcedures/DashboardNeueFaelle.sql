...
begin
  ...
  set @MD := (select max(Meldedatum) from Faelle where NeuerFall <> 0);
  SELECT
    @MD  AS Meldedatum  
  ...
  from (
    ...
    FROM Faelle  AS C WHERE NeuerFall <> 0
    UNION ALL
    SELECT 'Deaths'  AS Outcome, sum(AnzahlTodesfall) AS Counts
    FROM Faelle  AS C WHERE NeuerTodesfall <> 0
  )  AS F
  ;
...
begin
  ...
  SET @MD := (select max(Meldedatum) from Faelle);
  ...
  FROM (
    ...
    FROM Faelle  AS C WHERE NeuerFall > -1
    UNION ALL
    SELECT 
        'Deaths'  AS Outcome
